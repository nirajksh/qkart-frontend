...
const Register = () => {
  ...
  const register = async (formData) => {
       const collectFormData = {...formData}
        try{
          setIsLoading(true)
       const res = await axios.post(`${config.endpoint}/auth/register` ,collectFormData);
       const data = res.data;
          setIsLoading(false)
        console.log(data);
            ...
            <Stack spacing={2} className="form">
              ...
              />
               {isLoading ? <Box  display="flex"  justifyContent="center">
                     ...
                     : <Button className="button" variant="contained" onClick ={() => validateInput(registeredData)}>
                Register Now
              </Button>}
